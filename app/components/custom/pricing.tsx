...
export function PricingSection({
  ...
  return (
    <div
      className={cn(
        "flex w-full flex-col items-center justify-center space-y-5 p-4 pt-12",
        props.className
      )}
      {...props}
    >
  ...
  return (
    ...
    >
      {frequencies.map((freq) => (
        ...
        >
          <span className="relative z-10 text-black ">{freq}</span>
          {frequency === freq && (
            <motion.span
              layoutId="frequency"
              transition={{ type: "spring", duration: 0.4 }}
              className=" absolute  shadow-md inset-0 z-10 rounded-full "
            />
          )}
        </button>
      ))}
              ...
              <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                  <p
                    className={cn(
                      feature.tooltip &&
                        "cursor-pointer border-b border-border border-dashed"
                    )}
                  >
                    {feature.text}
                  </p>
